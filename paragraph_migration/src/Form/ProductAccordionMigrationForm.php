 ...
 */
class ProductAccordionMigrationForm extends FormBase {
  ...
  public function buildForm(array $form, FormStateInterface $form_state) {
    $form['migrate_productaccordion'] = array(
      '#type' => 'submit',
      '#value' => $this->t('Migrate product accordion'),
    );
  ...
  public function validateForm(array &$form, FormStateInterface $form_state) {
    ...
    $file = fopen($module_path . '/data/product_exports.csv', 'r');
    if (!$file) {
    ...
    if ($file) {
     ...
     // echo "</pre>";
      ...
      foreach ($paragraphs_array as $key => $value) {
        ...
        $batch['operations'][] = [
            '\Drupal\paragraph_migration\Controller\MigrateProductAccordion::migrateProductAccordion',
            array($value)
          ];
      }
